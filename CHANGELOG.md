...
- Moved `--serialization-format` option to be under the `serialize` command.
...
- Changed render model exporter manner of converting between coordinate systems;
    - Instead of manually rotating all positions, it now applies the rotation to a single unified root bone.

## 0.5.2
- Added ability to change the logging level.
- Overhauled the sound tag extractor; hopefully it is more accurate now.
- Added some debugging commands to debug builds.
