...
internal static class SoundLooping {
    internal static Command Register() {
        ...
        cmd.SetAction(ctx => {
            ...
            int trackOpt = ctx.GetValue(trackOption);
            bool playingAltTracks = ctx.GetValue(altTracksOption);
        ...
        try {
            ...
            while (phase != PlaybackPhase.Done) {
                ...
                string stage = phase switch {
                    PlaybackPhase.In => "in",
                    PlaybackPhase.Loop => exitTransitionRequested ? "loop, transitioning to exit..." : "loop",
