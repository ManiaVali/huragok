...
namespace Huragok.Application.Configuration;
...
internal static class ConfigurationReader {
    internal static string configFileLocation = string.Empty;
    ...
    private static ConfigurationObject ReadConfiguration() {
        ...
        if (string.IsNullOrWhiteSpace(configurationObject.ProjectPath))
            throw new ArgumentNullException($"{CONF_PARSE_ERR}: Key for `{Constants.ENGINE_PRETTY_NAME}` is missing or empty in configuration file.");

        if (!Directory.Exists(configurationObject.ProjectPath))
            throw new DirectoryNotFoundException($"{CONF_PARSE_ERR}: The editing kit location specified in key for `{Constants.ENGINE_PRETTY_NAME}` does not appear to exist.");
