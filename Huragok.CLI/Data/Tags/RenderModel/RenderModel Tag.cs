...
internal sealed class RenderModelTag : BaseTag<RenderModelFormat> {
    ...
    private float DistanceUnitScalar => this.distanceUnits switch {
        CoordinateUnit.Blam => 1,
        CoordinateUnit.JMS => Constants.WU_TO_JMS,
        CoordinateUnit.Metric => Constants.WU_TO_METERS,
    ...
    private ModelRoot DumpGLTF() {
        ...
        Logger.Debug($"{this.TagName}: {nameof(DumpGLTF)}: Using {this.distanceUnits} units (scalar {this.DistanceUnitScalar}f).");
        ...
        foreach (var node in this.armatureNodes) {
            ...
            gltfNode.SetLocalTransform(
                new AffineTransform(
                    ...
                    node.defaultTranslation * this.DistanceUnitScalar
                ), true);
        ...
        foreach (var region in this.RegionsByIndex.Values) {
            foreach (var perm in region.permutations) {
                ...
                for (int i = 0; i < exGeom.faces.Count; i++) {
                    ...
                    var vp1 = new VertexPositionNormal(exGeom.positions[face.Vertex1Index] * this.DistanceUnitScalar, exGeom.vtxNormals[face.Vertex1Index]);
                    var vp2 = new VertexPositionNormal(exGeom.positions[face.Vertex2Index] * this.DistanceUnitScalar, exGeom.vtxNormals[face.Vertex2Index]);
                    var vp3 = new VertexPositionNormal(exGeom.positions[face.Vertex3Index] * this.DistanceUnitScalar, exGeom.vtxNormals[face.Vertex3Index]);
                ...
                Matrix4x4 ComputeGlobal(ArmatureNode node) {
                    var local =
                        ...
                        Matrix4x4.CreateTranslation(node.defaultTranslation * this.DistanceUnitScalar);
            ...
            foreach (var marker in markers) {
                ...
                markerNode.SetLocalTransform(
                    new AffineTransform(
                        markerScale,
                        marker.rotation,
                        marker.translation * this.DistanceUnitScalar
                    ), true
                );
