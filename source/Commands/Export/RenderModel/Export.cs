    ...
    internal static class RenderModel {
        internal static Command Register() {
            ...
            cmd.SetHandler(ctx => {
                ...
                var modelExportContext = ctx.ParseResult.Resolve(modelHandler);
        ...
        private static void DumpRenderModelTagData(IEnumerable<string> tagFilePaths, string outDirectory, RenderModelFormat modelFormat, CoordinateUnit coordinateSystem) {
            ...
            foreach (string path in tagFilePaths) {
                ...
                var tagPath = TagPath.FromPathAndExtension(BlamFunctions.GetValidTagPath(path), "render_model");
                if (!BlamFunctions.ValidateTag(tagPath, "render_model")) throw new InvalidDataException($"Render model extraction failed; tag file `{path}` is invalid.");
            }
