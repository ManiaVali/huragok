...
internal abstract class BaseTag<TFileExt> : IDisposable where TFileExt : struct, Enum {
    ...
    internal BaseTag(TagPath tagPath) {
        ...
        if (!IsSubPathOf(ConfigurationReader.Configuration.ProjectPath, tagPath.RelativePathWithExtension))
    ...
    private static bool IsSubPathOf(string basePath, string fullPath) {
